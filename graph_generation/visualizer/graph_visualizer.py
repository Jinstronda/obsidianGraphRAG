    ...
    def load_data(self) -> bool:
        ...
        try:
            ...
            if not all(f.exists() for f in [docs_file, graph_file]):
                self.logger.error("Required cache files not found. Run csv_to_pickle_converter.py first.")
                return False
...
def main():
    ...
    cache_dir = script_dir / 'cache' / 'processed_data'
    visualizer = GraphVisualizer(cache_dir=str(cache_dir))
    
    # Load data
    print("📚 Loading cached graph data...")
    if not visualizer.load_data():
        print("❌ Failed to load data. Please run csv_to_pickle_converter.py first to process your vault.")
        return
