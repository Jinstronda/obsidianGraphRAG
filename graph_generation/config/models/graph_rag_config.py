...
from graph_generation.config.errors import LanguageModelConfigMissingError
from graph_generation.config.models.basic_search_config import BasicSearchConfig
from graph_generation.config.models.cache_config import CacheConfig
from graph_generation.config.models.chunking_config import ChunkingConfig
from graph_generation.config.models.cluster_graph_config import ClusterGraphConfig
from graph_generation.config.models.community_reports_config import CommunityReportsConfig
from graph_generation.config.models.drift_search_config import DRIFTSearchConfig
from graph_generation.config.models.embed_graph_config import EmbedGraphConfig
from graph_generation.config.models.extract_claims_config import ClaimExtractionConfig
from graph_generation.config.models.extract_graph_config import ExtractGraphConfig
from graph_generation.config.models.extract_graph_nlp_config import ExtractGraphNLPConfig
from graph_generation.config.models.global_search_config import GlobalSearchConfig
...
from graph_generation.config.models.local_search_config import LocalSearchConfig
from graph_generation.config.models.prune_graph_config import PruneGraphConfig
from graph_generation.config.models.reporting_config import ReportingConfig
from graph_generation.config.models.snapshots_config import SnapshotsConfig
from graph_generation.config.models.storage_config import StorageConfig
from graph_generation.config.models.summarize_descriptions_config import (
    SummarizeDescriptionsConfig,
)
from graph_generation.config.models.text_embedding_config import TextEmbeddingConfig
from graph_generation.config.models.umap_config import UmapConfig
from graph_generation.config.models.vector_store_config import VectorStoreConfig
from graph_generation.vector_stores.factory import VectorStoreType


class graph_generationConfig(BaseModel):
    ...
    root_dir: str = Field(
        ...
        default=graph_generation_config_defaults.root_dir,
    )
    ...
    models: dict[str, LanguageModelConfig] = Field(
        ...
        default=graph_generation_config_defaults.models,
    )
    ...
    def _validate_input_base_dir(self) -> None:
        ...
        if self.input.storage.type == defs.StorageType.file:
            if self.input.storage.base_dir.strip() == "":
                msg = "input storage base directory is required for file input storage. Please rerun `graph_generation init` and set the input storage configuration."
    ...
    def _validate_output_base_dir(self) -> None:
        ...
        if self.output.type == defs.StorageType.file:
            if self.output.base_dir.strip() == "":
                msg = "output base directory is required for file output. Please rerun `graph_generation init` and set the output configuration."
                raise ValueError(msg)
    ...
    outputs: dict[str, StorageConfig] | None = Field(
        ...
        default=graph_generation_config_defaults.outputs,
    )
    ...
    update_index_output: StorageConfig = Field(
        ...
        default=StorageConfig(
            base_dir=graph_generation_config_defaults.update_index_output.base_dir,
        ),
    )
    ...
    def _validate_update_index_output_base_dir(self) -> None:
        ...
        if self.update_index_output.type == defs.StorageType.file:
            if self.update_index_output.base_dir.strip() == "":
                msg = "update_index_output base directory is required for file output. Please rerun `graph_generation init` and set the update_index_output configuration."
                raise ValueError(msg)
    ...
    def _validate_reporting_base_dir(self) -> None:
        ...
        if self.reporting.type == defs.ReportingType.file:
            if self.reporting.base_dir.strip() == "":
                msg = "Reporting base directory is required for file reporting. Please rerun `graph_generation init` and set the reporting configuration."
                raise ValueError(msg)
    ...
    vector_store: dict[str, VectorStoreConfig] = Field(
        ...
        default_factory=lambda: {
            ...
            for k, v in graph_generation_config_defaults.vector_store.items()
        },
    )
    ...
    workflows: list[str] | None = Field(
        ...
        default=graph_generation_config_defaults.workflows,
    )
    ...
    extract_claims: ClaimExtractionConfig = Field(
        ...
        default=ClaimExtractionConfig(
            enabled=graph_generation_config_defaults.extract_claims.enabled,
        ),
    )
        ...
        for store in self.vector_store.values():
            if store.type == VectorStoreType.LanceDB:
                if not store.db_uri or store.db_uri.strip == "":
                    msg = "Vector store URI is required for LanceDB. Please rerun `graph_generation init` and set the vector store configuration."
                    raise ValueError(msg)
    ...
    def get_language_model_config(self, model_id: str) -> LanguageModelConfig:
        ...
        if model_id not in self.models:
            err_msg = f"Model ID {model_id} not found in configuration. Please rerun `graph_generation init` and set the model configuration."
    ...
    def get_vector_store_config(self, vector_store_id: str) -> VectorStoreConfig:
        ...
        if vector_store_id not in self.vector_store:
            err_msg = f"Vector Store ID {vector_store_id} not found in configuration. Please rerun `graph_generation init` and set the vector store configuration."
            raise ValueError(err_msg)
