...
class ApiKeyMissingError(ValueError):
    ...
    def __init__(self, llm_type: str, azure_auth_type: str | None = None) -> None:
        ...
        msg += ". Please rerun `graph_generation init` and set the API_KEY."
        super().__init__(msg)
...
class AzureApiBaseMissingError(ValueError):
    ...
    def __init__(self, llm_type: str) -> None:
        """Init method definition."""
        msg = f"API Base is required for {llm_type}. Please rerun `graph_generation init` and set the api_base."
        super().__init__(msg)
...
class AzureApiVersionMissingError(ValueError):
    ...
    def __init__(self, llm_type: str) -> None:
        """Init method definition."""
        msg = f"API Version is required for {llm_type}. Please rerun `graph_generation init` and set the api_version."
        super().__init__(msg)
...
class AzureDeploymentNameMissingError(ValueError):
    ...
    def __init__(self, llm_type: str) -> None:
        """Init method definition."""
        msg = f"Deployment name is required for {llm_type}. Please rerun `graph_generation init` set the deployment_name."
        super().__init__(msg)
...
class LanguageModelConfigMissingError(ValueError):
    ...
    def __init__(self, key: str = "") -> None:
        """Init method definition."""
        msg = f'A {key} model configuration is required. Please rerun `graph_generation init` and set models["{key}"] in settings.yaml.'
        super().__init__(msg)
