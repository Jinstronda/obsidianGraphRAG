...
from graph_generation.cache.noop_pipeline_cache import NoopPipelineCache
from graph_generation.cache.pipeline_cache import PipelineCache
from graph_generation.config.enums import AsyncType
from graph_generation.index.operations.build_noun_graph.np_extractors.base import (
    BaseNounPhraseExtractor,
)
from graph_generation.index.utils.derive_from_rows import derive_from_rows
from graph_generation.index.utils.graphs import calculate_pmi_edge_weights
from graph_generation.index.utils.hashing import gen_sha512_hash
...
def _extract_edges(
    ...
    # Debug: print first few rows and types
    print("First few rows of edge_df['edges'] before assignment:")
    print(edge_df["edges"].head())
    print("Types:", edge_df["edges"].apply(type).value_counts())

    # Filter: only keep rows where 'edges' is a tuple/list of length 2
    edge_df = edge_df[edge_df["edges"].apply(lambda x: isinstance(x, (tuple, list)) and len(x) == 2)]
